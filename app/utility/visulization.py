...
class Visualization:
    ...
    @staticmethod
    def plot_array_real_time(u_array, x_vec, t_vec, dt, y_min, y_max, step=10):
        """
        Plots the temperature distribution in real-time for each time step for multiple configurations.
        
        Parameters:
        u_array : 3D array
            Array of temperature distributions, where each 2D slice corresponds to a different configuration
            (dimensions: [configurations, time steps, spatial points]).
        x_vec : 1D array
        ...
        step: integer
            The step speed of the graph in real-time.
        """
        num_configs = u_array.shape[0]  # Number of configurations
        
        for t in range(0, len(t_vec) - 1, step):
            for config in range(num_configs):
                pyplot.plot(x_vec, u_array[config, t], label=f"Config {config + 1}")
        ...
        for config in range(num_configs):
            pyplot.plot(x_vec, u_array[config, -1], label=f"Config {config + 1}")
